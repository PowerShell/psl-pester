...
Describe "New-Fixture" {
    ...
    Context "Name and Path parameter is specified:" {
        ...
        It "Creates fixture in full Path:" {
            $name = "Test-Fixture"
            $path = Join-Path -Path $TestDrive -ChildPath full
        ...
        It "Creates fixture in relative Path:" {
            ...
            Join-Path -Path (Join-Path -Path $path -ChildPath relative) -ChildPath "$name.ps1" | Should Exist
            Join-Path -Path (Join-Path -Path $path -ChildPath relative) -ChildPath "$name.Tests.ps1" | Should Exist
        }
