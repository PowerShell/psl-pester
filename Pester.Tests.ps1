...
$manifestPath   = "$here${DirectorySeparatorChar}Pester.psd1"
$changeLogPath = "$here${DirectorySeparatorChar}CHANGELOG.md"
...
Describe 'Style rules' {
    ...
    $files = @(
        ...
        Get-ChildItem $pesterRoot${DirectorySeparatorChar}Functions -Include *.ps1,*.psm1 -Recurse
    )
...
InModuleScope Pester {
    Describe 'ResolveTestScripts' {
        ...
        It 'Resolves non-wildcarded file paths regardless of whether the file ends with Tests.ps1' {
            $result = @(ResolveTestScripts $TestDrive${DirectorySeparatorChar}SomeOtherFile.ps1)
            $result.Count | Should Be 1
            $result[0].Path | Should Be "$TestDrive${DirectorySeparatorChar}SomeOtherFile.ps1"
        }

        It 'Finds only *.Tests.ps1 files when the path contains wildcards' {
            $result = @(ResolveTestScripts $TestDrive${DirectorySeparatorChar}*.ps1)
            $result.Count | Should Be 2

            $paths = $result | Select-Object -ExpandProperty Path

            ($paths -contains "$TestDrive${DirectorySeparatorChar}SomeFile.Tests.ps1") | Should Be $true
            ($paths -contains "$TestDrive${DirectorySeparatorChar}SomeOtherFile.Tests.ps1") | Should Be $true
        }
        ...
        It 'Assigns empty array and hashtable to the Arguments and Parameters properties when none are specified by the caller' {
            $result = @(ResolveTestScripts "$TestDrive${DirectorySeparatorChar}SomeFile.ps1")

            $result.Count | Should Be 1
            $result[0].Path | Should Be "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"
        ...
        Context 'Passing in Dictionaries instead of Strings' {
            It 'Allows the use of a "P" key instead of "Path"' {
                $result = @(ResolveTestScripts @{ P = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1" })
            ...
            It 'Allows the use of an "Arguments" key in the dictionary' {
                $result = @(ResolveTestScripts @{ Path = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"; Arguments = $testArgs })
            ...
            It 'Allows the use of an "Args" key in the dictionary' {
                $result = @(ResolveTestScripts @{ Path = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"; Args = $testArgs })
            ...
            It 'Allows the use of an "A" key in the dictionary' {
                $result = @(ResolveTestScripts @{ Path = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"; A = $testArgs })
            ...
            It 'Allows the use of a "Parameters" key in the dictionary' {
                $result = @(ResolveTestScripts @{ Path = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"; Parameters = $testParams })
            ...
            It 'Allows the use of a "Params" key in the dictionary' {
                $result = @(ResolveTestScripts @{ Path = "$TestDrive${DirectorySeparatorChar}SomeFile.ps1"; Params = $testParams })
